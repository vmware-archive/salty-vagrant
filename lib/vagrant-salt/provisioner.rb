module VagrantSalt
  class Provisioner < Vagrant::Provisioners::Base
    class Config < Vagrant::Config::Base
      ...
      attr_accessor :git_install_treeish
      ...
      def bootstrap_options
        if @git_install_treeish
          ' -t ' + @git_install_treeish + ' git'
        else
          ''
        end 
      end
    ...
    def bootstrap_salt_minion
      ...
      bootstrap = env[:vm].channel.sudo("/tmp/bootstrap-salt-minion.sh" + config.bootstrap_options) do |type, data|
        if data[0] == "\n"
